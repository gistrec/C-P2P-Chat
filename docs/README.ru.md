...
| Флаг | Описание |
| ---- | -------- |
| `-name <ник>` | Установка ника (**обязательно**) |
| `-connect <ip> <port>` | Подключиться к узлу |
| `-port <port>` | Локальный порт (по умолчанию `8888`) |

Пример:
```bash
./build/C_P2P_Chat -name Сашка -connect 46.180.227.50 8888 -port 8080
```
